...
function (
	...
	return declare([WidgetBase, TemplatedMixin, WidgetsInTemplateMixin], {
			...
			this.tree = new Tree({
				...
				onClick: function (item) {
					if (item.directory == false) {
						...
						topic.publish(appTopics.filter.jobQueriesChanged, this, { selectedId: item.id });
						//hide drop down
		...
		setQueryStore: function (publicQueries, userQueries) {
			...
			//publish query
			topic.publish(appTopics.filter.jobQueriesChanged, this, { selectedId: self.initialQueryId });
		},
