define([
	...
	function(
		...
		Map, Popup, Attribution, OverviewMap, Scalebar, FeatureLayer, webMercatorUtils, Polygon, Extent, SpatialReference, GraphicsLayer, SimpleFillSymbol, SimpleLineSymbol, QueryTask, Query,
	...
	// main geolocation widget
		return declare("widget.gis.EsriMap", [_WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin], {
			...
			postCreate: function() {
				...
				this.map = new esri.Map(this.mapId, {
					basemap: this.mapConfig.defaultBasemap,
			...
			startup: function () {
				...
				on(this.map.infoWindow.domNode, "a:click", function (event) {
					var featureId = event.srcElement.attributes["data-feature-id"].value;
					
					if (event.srcElement.attributes["data-feature-single"].value == "true") {
			...
			initTopics: function() {
				...
				topic.subscribe(appTopics.map.draw.start, function () {
					var layer = self.map.getLayer(self.featureLayerId);
			...
			getLayerObject: function(layer) {
				...
				this.featureLayerId = layer.options.id;
				...
				l.on("click", function(e) {
					self.screenPoint = e.screenPoint;
					executeQueryTask(e);
				});

				var jobQueryContent = "";
				
				topic.subscribe("Map/ClickLayer/JobQuery", function(args, jobStatuses) {
					jobQueryContent = "";
				...
				query.outFields = ["OBJECTID", self.jobIdField, "OBJECT_MODIFIED", "LAST_SYNC"];

				var symbol = new esri.symbol.SimpleFillSymbol(esri.symbol.SimpleFillSymbol.STYLE_SOLID, new esri.symbol.SimpleLineSymbol(esri.symbol.SimpleLineSymbol.STYLE_SOLID, new dojo.Color([255, 0, 0]), 1), new dojo.Color([255, 0, 0, 0.5]));
				...
				function showFeature(feature, evt) {
					...
					topic.publish("Map/ClickLayer", feature.attributes[self.jobIdField]);
				}

				this.singleFeature = showFeature;
		...
		refreshLayers: function () {
		    var layer = this.map.getLayer(this.featureLayerId);
		    layer.refresh();
		},

		getUpdatedFeatures: function (data) {
		    var layer = this.map.getLayer(this.featureLayerId);
			layer.setDefinitionExpression(this.jobIdField + " IN (" + data + ")");
			this.setMapExtent();
			this.refreshLayers();
		},
